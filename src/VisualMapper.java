	...
	class ControlPanel extends JPanel {
		...
		class SliderListener1 implements ChangeListener {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (!source.getValueIsAdjusting()) {
					VisualMapper.this.TX = source.getValue();
					VisualMapper.this.sp.repaint();
				}
			}
		}
		class SliderListener2 implements ChangeListener {
			...
			public void stateChanged(ChangeEvent e) {
				JSlider source = (JSlider) e.getSource();
				if (!source.getValueIsAdjusting()) {
					VisualMapper.this.TY = source.getValue();
					VisualMapper.this.sp.repaint();
				}
			}
		}
		//follow the classes below to make the gui
					...
					public void mousePressed(MouseEvent e) {
						try {
							int a = Integer.parseInt(ControlPanel.this.In
									.getText());
							b.insert(a);//add graph inset
							VisualMapper.this.sp.repaint();
							VisualMapper.this.cs
									.setText("#A Node "
											+ a
	...
	class Panel extends JPanel {
		...
		public void paintComponent(Graphics comp) {
			...
			if (b.root != null) {
				...
				n.drawOn(comp2D);
				this.drawRec(n, root, comp2D, VisualMapper.this.TX,
						VisualMapper.this.TY);
			} else {// empty//
			}
		}
